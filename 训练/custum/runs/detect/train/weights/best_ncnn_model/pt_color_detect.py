...
PT_MODEL = "best.pt"  # 替换为你的pt模型路径
INPUT_SIZE = 416  # 与ncnn一致
...
while True:
    ...
    t1 = time.time()
    print(f"pt 推理耗时: {(t1-t0)*1000:.2f} ms")
    ...
    for det in results.boxes.data.cpu().numpy():
        ...
        if score < 0.85:
            continue
        # 坐标缩放回原frame
        x1 = int(x1 / INPUT_SIZE * frame.shape[1])
        y1 = int(y1 / INPUT_SIZE * frame.shape[0])
        x2 = int(x2 / INPUT_SIZE * frame.shape[1])
        y2 = int(y2 / INPUT_SIZE * frame.shape[0])
